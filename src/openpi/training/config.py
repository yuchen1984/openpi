...
_CONFIGS = [
    ...
    #
    # AIMED-MIX variant (2026-07-07 RL-exploration follow-up). Same
    # model/loss/schedule as pi05_base_sim_finetune_negY_flipcheck; the
    # only change is the dataset: 150 FIXED-template AIMED episodes
    # (data/episodes_cloth_aimed_v1, mean IoU 0.828) MIXED with the 201
    # negY_flipcheck episodes -> local/cloth_aimed_mix_351ep. The aimed
    # data supplies the precise-placement/aiming skill the re-placed-
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/cloth_aimed_mix_351ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    # Recipe = real_cube_v4 (gripper EE task): image-only (discrete_state_input
    # =False), gripper dim-6 loss weight 2.0; 24k schedule, permanent ckpt every 4k.
    #
    TrainConfig(
    ...
    # (subsumes --canonicalize-ori-deltas); extra_delta_transform=True re-derives
    # chunk-relative deltas. Deploy with the GUI's chunkwise consumption path.
    #
    TrainConfig(
        name="pi05_base_finetune_sleeve_lift_v1_chunkwise",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,          # image-only, as real_cube_v4 (gripper EE task)
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/sleeve_lift_v1_66ep_chunkwise",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=True,
            action_dim=8,
        ),
    ...
    #
    # Real NERO YELLOW sleeve pick-and-lift from MID-AIR (the sleeve hangs from a
    # jacket; grasp at z 0.19-0.42 m, not from the table). nero-exp-lf-haptic
    # sleeve_lift_yellow_v0: all 68 eps kept (every one ends with the sleeve
    # held) / 14,461 frames. Same dead-follower-gripper situation and the same
    # conversion as sleeve_lift_v1, plus a prompt override:
    #   scripts/convert_real_v3_to_openpi.py --src sleeve_lift_yellow_v0/uf850/libero
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/sleeve_lift_yellow_v0_68ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    # "pick up and lift the sleeve" (exactly 50/50). Trained ONLY if the
    # pre-registered prompt-sensitivity rule (uf850-branch2 scripts/psens_decide.py)
    # finds the colour-prompt model fragile to a generic / unseen-colour prompt.
    # Same recipe and 24k steps, so the two compare at equal sample count.
    #
    TrainConfig(
        name="pi05_base_finetune_sleeve_lift_ybp_locked_aug",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,          # image-only, as real_cube_v4 (gripper EE task)
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/sleeve_lift_ybp_locked_aug_236ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    #
    TrainConfig(
        name="pi05_base_finetune_sleeve_lift_yellow_v0_chunkwise",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,          # image-only, as real_cube_v4 (gripper EE task)
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/sleeve_lift_yellow_v0_68ep_chunkwise",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=True,
            action_dim=8,
        ),
    ...
    #   V2 72d  = V1 + 64 tactile taxels (Index/Middle/Ring/Pinky, 16 each)
    #   V3 79d  = V2 + 7 joint torques        (v1 recording ONLY — v0 has no torque)
    # The THUMB pad is EXCLUDED: 0/16 of its taxels ever fire in either recording, so
    # it is 16 constant dims and ~60 wasted prompt tokens carrying zero information
    # (converter: --index-tactile fingers4).
    ...
    #   [--index-tactile fingers4] [--joint-torque on]
    ...
    # 72/79-dim states need 275/312 tokens -> 384 (the 8-dim V1 fits the 200 default).
    #
    TrainConfig(
    ...
    TrainConfig(
        name="pi05_base_finetune_nudge_template_tac4",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_v01_tac4_111ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    TrainConfig(
        name="pi05_base_finetune_nudge_template_tac4_torque",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,      # 79-dim = LIBERO 8 + 64 taxels + 7 torques
            max_token_len=384,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ),
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_v1_tac4_torque_51ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    #
    # AUGMENTED round (2026-07-15): fold nudge_template_v4 (21 eps) into the same
    # three-way A/B. v2/v3 are NOT usable — they have no uf850/libero EE-pose export
    # (only a joint-space top-level export) and no frames.npz to rebuild it offline.
    # v0 still lacks torque, so the torque run stays v1+v4. Data grew ~1.2-1.5x, so
    # the schedule is bumped 30k -> 40k (num_train_steps AND decay_steps). Everything
    # else is identical to the v0+v1 configs above; the originals are left intact.
    #   V1  8d  = v0(ex12)+v1+v4  (132 eps)
    #   V2 72d  = V1 + 64 four-finger taxels (fingers4)  (132 eps)
    #   V3 79d  = V2 + 7 joint torques  (v1+v4 only, 72 eps)
    #
    TrainConfig(
        name="pi05_base_finetune_nudge_template_aug",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,      # 8-dim LIBERO proprio state
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_aug_132ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    TrainConfig(
        name="pi05_base_finetune_nudge_template_tac4_aug",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,      # 72-dim = LIBERO 8 + 64 taxels (no dead thumb)
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_aug_tac4_132ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
    ...
    #
    # SMOOTHED-ACTIONS variant of the chunkwise tac4 _aug config (2026-07-22,
    # uf850-branch docs/gemini_vla_jitter_assessment.md). Dataset
    # local/nudge_template_aug_tac4_132ep_chunkwise_smoothed = the SAME sources
    # converted with --delta-mode chunkwise --smooth-actions savgol (w=5, k=3):
    # the action targets are derived from a Savitzky-Golay-smoothed copy of the
    # EE pose trajectory (BC stops memorizing teleop tremor — the data-side
    # jerk lever; the chunkwise A/B showed the residual wiggle is in what the
    # policy COMMANDS, i.e. learned), while the state channel stays raw
    # (deployment parity). Single-variable A/B vs
    # pi05_base_finetune_nudge_template_tac4_aug_chunkwise. Same norm rules:
    # quantile + patch_norm_stats_extras.py for the tactile dims.
    #
    TrainConfig(
        name="pi05_base_finetune_nudge_template_tac4_aug_chunkwise_smoothed",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,      # 72-dim = LIBERO 8 + 64 taxels (no dead thumb)
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_aug_tac4_132ep_chunkwise_smoothed",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=True,
            action_dim=8,
        ),
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="local/nudge_template_aug_tac4_torque_72ep",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            action_dim=8,
        ),
